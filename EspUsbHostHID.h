...
public:
  ...
  virtual void shutDownUPS() {
    // Ejemplo para un APC Smart-UPS 750 (ajusta según tu modelo):
    uint8_t shutdown_command[1] = { 0 };
    shutdown_command[0] = 0x39;  // Report ID 01
    //shutdown_command[1] = 0x39;  //0B
    //shutdown_command[2] = 0x00;  //01
    //shutdown_command[3] = 0x72;  // Report ID
    //shutdown_command[4] = 0x00;
    //shutdown_command[5] = 0x00;
    //shutdown_command[6] = 0x00;  // Report ID
    //shutdown_command[7] = 0x00;
    

    //submit_control(0x21, 0x09, 0x0201, 0x0000, 1, &shutdown_command);
    esp_err_t result = submit_control(0x21, 0x09, 0x0200 | 0x39, 0x0000, sizeof(shutdown_command), &shutdown_command);
    //esp_err_t result = submit_control(0x21, 0x09, 0x0216);
    ESP_LOGI("Comando shutdow enviado...", "%d", result);
  }
...
protected:
  ...
  EspUsbHostHID(uint8_t InterfaceProtocol)
      ...
      case USB_B_DESCRIPTOR_TYPE_INTERFACE:
        {
          ...
          } else if ((intf->bInterfaceClass == USB_CLASS_HID) && (intf->bInterfaceSubClass == 0x00) && (intf->bInterfaceProtocol == InterfaceProtocol)) {
            esp_err_t err = usb_host_interface_claim(this->clientHandle, this->deviceHandle, intf->bInterfaceNumber, intf->bAlternateSetting);
            if (err != ESP_OK) {
              ESP_LOGI("EspUsbHostHID", "usb_host_interface_claim() err=%x", err);
            } else {
  ...
  virtual bool isReady() override {
    return usbTransfer && reportDescriptor.size();
  }
